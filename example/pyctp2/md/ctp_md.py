...
class MdSpiDelegate(MdSpi):
    ...
    def market_data2tick(self,market_data):
        ...
        try:
            ...
            #self.logger.warning(u'MD:行情数据:%s' % market_data)
            if not rev.is_valid():
                raise ValueError("tick not valid")
        except Exception as inst:
            self.logger.warning(u'MD:行情数据转换错误:%s,赋值进程=%s' % (str(inst),state))
            self.logger.warning(u'MD:行情数据转换错误,源记录:%s' % market_data)
