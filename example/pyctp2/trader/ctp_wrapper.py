...
class TraderSpiDelegate(TraderSpi):
    ...
    def query_settlement_info(self):
        ...
        self.logger.info(u'TD:取上一日结算单信息并确认, BrokerID=%s, investorID=%s' % (self._broker, self._investor))
    ...
    def fetch_instrument_marginrate(self, instrument_id):
        ...
        logging.info(u'A:查询保证金率, 函数发出返回值:%s' % ret)
        return ret
    ...
    def fetch_instrument(self, instrument_id):
        ...
        logging.info(u'A:查询合约, 函数发出返回值:%s' % ret)
        return ret
    ...
    def fetch_depth_market_data(self, instrument_id):
        ...
        logging.info(u'A:查询合约行情, 函数发出返回值:%s' % ret)
        return ret
    ...
    def OnRspQryDepthMarketData(self, pDepthMarketData, pRspInfo, nRequestID, bIsLast):
        ...
        """
        self.logger.info(u'TD:查询行情响应:%s' % pDepthMarketData)
        if bIsLast and self.isRspSuccess(pRspInfo):
    ...
    def OnErrRtnOrderInsert(self, pInputOrder, pRspInfo):
        ...
        self.logger.error(u'TD:交易所报单录入错误回报, 正常后不应该出现, rspInfo=%s'%(str(pRspInfo), ))
    ...
    def OnRtnOrder(self, pOrder):
        ...
        #self.logger.info(u'报单响应 repr, Order=%s' % repr(pOrder))
        #print("ORO:",pOrder.ExchangeID)
        self.logger.info(u'报单响应 str, Order=%s' % str(pOrder))
        if pOrder.FrontID == self.login_info.front_id and pOrder.SessionID != self.login_info.session_id:
            ...
            #CTP接受，但未发到交易所  pOrder.OrderSubmitStatus == UType.THOST_FTDC_OSS_InsertSubmitted.  未必!!!.成交时也是已提交状态
        ...
        elif pOrder.OrderStatus == UType.THOST_FTDC_OST_Unknown and pOrder.OrderSysID != '':     #交易所接受后,得到OrderSysID
