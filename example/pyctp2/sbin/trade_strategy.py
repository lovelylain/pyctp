...
class SExampleA(BaseStrategy):
    ...
    def check_open(self,ctick,CA):
        ...
        self.ntick += 1
        #print("in check_open,%s:ntick=%d" %(ctick.instrument,self.ntick))
        if self.is_full:
            #print("A: is full")
            return []
        ...
        logging.info("SExampleA %s check_open time:%d,ntick=%d,cur_times=%d",ctick.instrument,ctick.time,self.ntick,self.cur_times)
    ...
    def calc_open_volume(self,popen):
        ...
        if clen == 0:
            ...
            print("volume =",v)
            popen.unit = popen.planned = v if v>0 else 1
    ...
    def _check_close(self,ctick,position,params,CA):
        ...
        logging.info("发出平仓指令，%s 平仓 %d 手， 基准价格 %02f, 超越跳数 %d",ctick.instrument,position.volume_holding,cur_price,cmd.extra_hops)
        self.is_full = False
        return [cmd]
...
class SExampleAB(BaseStrategy):
    ...
    def check_open(self,ctick,CA,CB):
        ...
        #print("in check open")
        self.ntick += 1
        if self.is_full or self.ntick < 30 or self.ntrade >= self.max_trade:
            return []
        ...
        cmd2 = POPEN(contract=CB,direction=SHORT,base_price=CB.last_price,closers=[closer],extra_hops=30) #targe_price=baseprice+hops*tick,extra_hops=MAX_VALUE/-MAX_VALUE则为停板
        return [cmd1,cmd2]
    ...
    def calc_open_volume(self,instructionA,instructionB):
        ...
        if ca_len == 0:
            ...
            times = int(self.available_balance // (margin_a + margin_b)) // 10
            instructionA.unit = instructionA.planned = 1.2 * times
            instructionB.unit = instructionB.planned = 1 * times
            if instructionA.unit < 1:
                instructionA.unit = instructionA.planned = 1
            if instructionB.unit < 1:
                instructionB.unit = instructionB.planned = 1
