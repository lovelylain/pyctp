...
TRepayShortSellAlgo = c_char #买券还券算法
SSA_Original = '0' #默认算法
SSA_Ratio = '1' #按还券比例计算
SSA_Min = '2' #Min[1,2]
TINTEGER = c_int #一般整型
...
TAutoIncrement = c_int #自增长序号
TInstrumentRange = c_char #股票权限分类
INR_All = '1' #所有
INR_Product = '2' #产品
INR_Model = '3' #股票权限模版
INR_Stock = '4' #股票
TRightModelID = c_char * 31 #股票权限模版代码
TRightModelName = c_char * 161 #股票权限模版名称
TRightModelMemo = c_char * 1025 #股票权限模版备注
TYear1 = c_int #年份
TMonth1 = c_int #月份
...
class Instrument(BaseStruct): #合约
    _fields_ = (
        ...
        ('RightModelID', TRightModelID), #股票权限模版代码, char[31]
    )
...
class TradingAccount(BaseStruct): #资金账户
    _fields_ = (
        ...
        ('SSStockValue', TMoney), #融券总市值, double
        ('BondRepurchaseValue', TMoney), #国债回购占用资金, double
    )
...
class InstrumentCommissionRate(BaseStruct): #合约手续费率
    _fields_ = (
        ...
        ('StampTaxRateByMoney', TRatio), #印花税率, double
        ('StampTaxRateByVolume', TRatio), #印花税率(按手数), double
        ('TransferFeeRateByMoney', TRatio), #过户费率, double
        ('TransferFeeRateByVolume', TRatio), #过户费率(按手数), double
        ('TradeFeeByMoney', TRatio), #交易费, double
        ('TradeFeeByVolume', TRatio), #交易费(按手数), double
        ('MarginByMoney', TRatio), #交易附加费率, double
...
class InstrumentTradingRight(BaseStruct): #投资者合约交易权限
    _fields_ = (
        ...
        ('ExchangeID', TExchangeID), #交易所代码, char[9]
        ('InstrumentRange', TInstrumentRange), #股票权限分类, char
    )
