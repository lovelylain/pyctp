...
cdef extern _init(self, m):
    ...
    ApiStruct_InputLock = XGotAttr(XGetAttr(m, S_InputLock), fa)
    ...
    ApiStruct_Lock = XGotAttr(XGetAttr(m, S_Lock), fa)
    ApiStruct_LockPosition = XGotAttr(XGetAttr(m, S_LockPosition), fa)
...
cdef class TraderApi:
    ...
    def ReqLockInsert(self, pInputLock, int nRequestID):
        cdef size_t address
        if self.spi is NULL: return
        address = ApiStruct_addressof(pInputLock)
        with nogil: nRequestID = self.api.ReqLockInsert(<CInputLockField *>address, nRequestID)
        return nRequestID
    ...
    def ReqQryLock(self, pQryLock, int nRequestID):
        cdef size_t address
        if self.spi is NULL: return
        address = ApiStruct_addressof(pQryLock)
        with nogil: nRequestID = self.api.ReqQryLock(<CQryLockField *>address, nRequestID)
        return nRequestID

    def ReqQryLockPosition(self, pQryLockPosition, int nRequestID):
        cdef size_t address
        if self.spi is NULL: return
        address = ApiStruct_addressof(pQryLockPosition)
        with nogil: nRequestID = self.api.ReqQryLockPosition(<CQryLockPositionField *>address, nRequestID)
        return nRequestID
...
cdef extern int TraderSpi_OnRspLockInsert(self, CInputLockField *pInputLock, CRspInfoField *pRspInfo, int nRequestID, cbool bIsLast) except -1:
    self.OnRspLockInsert(None if pInputLock is NULL else ApiStruct_InputLock(<size_t>pInputLock), None if pRspInfo is NULL else ApiStruct_RspInfo(<size_t>pRspInfo), nRequestID, bIsLast)
    return 0
...
cdef extern int TraderSpi_OnRspQryLock(self, CLockField *pLock, CRspInfoField *pRspInfo, int nRequestID, cbool bIsLast) except -1:
    self.OnRspQryLock(None if pLock is NULL else ApiStruct_Lock(<size_t>pLock), None if pRspInfo is NULL else ApiStruct_RspInfo(<size_t>pRspInfo), nRequestID, bIsLast)
    return 0

cdef extern int TraderSpi_OnRspQryLockPosition(self, CLockPositionField *pLockPosition, CRspInfoField *pRspInfo, int nRequestID, cbool bIsLast) except -1:
    self.OnRspQryLockPosition(None if pLockPosition is NULL else ApiStruct_LockPosition(<size_t>pLockPosition), None if pRspInfo is NULL else ApiStruct_RspInfo(<size_t>pRspInfo), nRequestID, bIsLast)
    return 0
...
cdef extern int TraderSpi_OnRtnLock(self, CLockField *pLock) except -1:
    self.OnRtnLock(None if pLock is NULL else ApiStruct_Lock(<size_t>pLock))
    return 0

cdef extern int TraderSpi_OnErrRtnLockInsert(self, CInputLockField *pInputLock, CRspInfoField *pRspInfo) except -1:
    self.OnErrRtnLockInsert(None if pInputLock is NULL else ApiStruct_InputLock(<size_t>pInputLock), None if pRspInfo is NULL else ApiStruct_RspInfo(<size_t>pRspInfo))
    return 0
