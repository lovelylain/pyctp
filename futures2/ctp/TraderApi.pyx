...
cdef extern _init(self, m):
    ...
    ApiStruct_BatchOrderAction = XGotAttr(XGetAttr(m, S_BatchOrderAction), fa)
    ...
    ApiStruct_InputBatchOrderAction = XGotAttr(XGetAttr(m, S_InputBatchOrderAction), fa)
    ...
    ApiStruct_ProductGroup = XGotAttr(XGetAttr(m, S_ProductGroup), fa)
...
cdef class TraderApi:
    ...
    def ReqBatchOrderAction(self, pInputBatchOrderAction, int nRequestID):
        cdef size_t address
        if self.spi is NULL: return
        address = ApiStruct_addressof(pInputBatchOrderAction)
        with nogil: nRequestID = self.api.ReqBatchOrderAction(<CInputBatchOrderActionField *>address, nRequestID)
        return nRequestID
    ...
    def ReqQryProductGroup(self, pQryProductGroup, int nRequestID):
        cdef size_t address
        if self.spi is NULL: return
        address = ApiStruct_addressof(pQryProductGroup)
        with nogil: nRequestID = self.api.ReqQryProductGroup(<CQryProductGroupField *>address, nRequestID)
        return nRequestID
...
cdef extern int TraderSpi_OnRspBatchOrderAction(self, CInputBatchOrderActionField *pInputBatchOrderAction, CRspInfoField *pRspInfo, int nRequestID, cbool bIsLast) except -1:
    self.OnRspBatchOrderAction(None if pInputBatchOrderAction is NULL else ApiStruct_InputBatchOrderAction(<size_t>pInputBatchOrderAction), None if pRspInfo is NULL else ApiStruct_RspInfo(<size_t>pRspInfo), nRequestID, bIsLast)
    return 0
...
cdef extern int TraderSpi_OnRspQryProductGroup(self, CProductGroupField *pProductGroup, CRspInfoField *pRspInfo, int nRequestID, cbool bIsLast) except -1:
    self.OnRspQryProductGroup(None if pProductGroup is NULL else ApiStruct_ProductGroup(<size_t>pProductGroup), None if pRspInfo is NULL else ApiStruct_RspInfo(<size_t>pRspInfo), nRequestID, bIsLast)
    return 0
...
cdef extern int TraderSpi_OnErrRtnBatchOrderAction(self, CBatchOrderActionField *pBatchOrderAction, CRspInfoField *pRspInfo) except -1:
    self.OnErrRtnBatchOrderAction(None if pBatchOrderAction is NULL else ApiStruct_BatchOrderAction(<size_t>pBatchOrderAction), None if pRspInfo is NULL else ApiStruct_RspInfo(<size_t>pRspInfo))
    return 0
